...
MCP-сервер к API Яндекс Метрики с **полным покрытием**: 108 методов — 108 инструментов.
...
| API | методов | примеры инструментов |
| --- | ---: | --- |
| Management | 95 (21 ресурс) | `metrika_counter_list`, `metrika_goal_create`, `metrika_segment_update` |
...
| `METRIKA_ALLOW_WRITES` | не задана | `1` разрешает вызовы, меняющие данные. Пока не задана — 57 таких инструментов видны, но вызов возвращает отказ `writes_disabled`. |
| `METRIKA_TOOLS` | пусто (все 108) | Отбор через запятую: раздел (`stat`, `logs`, `management`), префикс имени (`metrika_goal`) или точное имя. |
...
Полный набор инструментов стоит **≈50 тысяч токенов** в `tools/list` — это едет в контекст
каждой сессии. Если Management-раздел не нужен, `METRIKA_TOOLS=stat,logs` сокращает набор
до 13 инструментов.

## Безопасность

- **Запись выключена по умолчанию.** Среди методов четырнадцать `DELETE` и пять удаляющих
  `POST` (`.../measurement/delete`, `.../expense/delete`, `.../logrequest/{id}/clean` и т. д.).
  Цена ошибочного вызова — удалённый счётчик или цель без возможности восстановить историю,
  поэтому запись включается отдельным решением оператора.
...
```bash
npm test          # 43 теста: спека, схемы инструментов, протокол MCP
