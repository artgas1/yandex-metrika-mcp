...
export class YandexMetrikaClient {
     ...
     */
    buildDimensionNameWithAttribution(dimension, attribution = 'automatic') {
        ...
        if (!attributionPrefixMap[attribution]) {
            throw new Error(`Invalid attribution: ${attribution}. Must be one of: ${Object.keys(attributionPrefixMap).join(', ')}`);
        }
    ...
    async getDataByTime(counterId, metrics, dateFrom, dateTo, dimensions, group = 'day', topKeys = 7, timezone, attribution) {
        if (!counterId || typeof counterId !== 'string') {
            throw new Error('Counter ID must be a non-empty string');
        }
        ...
        if (attribution && dimensions && dimensions.length > 0) {
            processedDimensions = dimensions.map(dim => this.buildDimensionNameWithAttribution(dim, attribution));
        }
