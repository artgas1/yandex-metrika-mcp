 ...
 * ByteString. Символ вне ASCII роняет сам `fetch` — до сети дело не доходит.
 *
 * Отказ при этом получается невнятный вдвойне. Во-первых, сообщение
 * («Cannot convert argument to a ByteString…») не упоминает ни токен, ни заголовок.
 * Во-вторых, повтор по ошибке считает это сетевым сбоем и честно ждёт три секунды,
 * прежде чем сдаться, — так отказ окончательно перестаёт быть похожим на свою причину.
 *
 * Промах не выдуманный: на русской раскладке `с`, `е`, `о`, `р`, `а`, `х` неотличимы
 * на вид от латинских, и одна такая буква попадает в токен при наборе руками.
 *
 * Замерено 09.09.2026, чтобы не сторожить того, чего нет: перевод строки и пробел
 * по краям `undici` срезает сам, запрос с ними проходит. Отказывает ровно не-ASCII.
 */
  ...
  for (let i = 0; i < token.length; i++) {
    ...
    if (code > 0x7f) {
      ...
      return (
        ...
        'он не ASCII, и такой заголовок Authorization не собирается вовсе. ' +
        'Чаще всего это кириллическая буква, неотличимая на вид от латинской ' +
        '(с, е, о, р, а, х), или длинное тире вместо дефиса. Наберите токен заново ' +
        'или скопируйте его из источника, а не из переписки.'
      );
    }
  }
  return null;
